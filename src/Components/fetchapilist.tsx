...
const ApiKeyList = () => {
  ...
  return (
    <div className="max-w-md mx-auto p-6 border border-gray-300 rounded-lg shadow-lg bg-white">
      ...
      {loading ? /*{<IconLoader className=' text-center mx-auto size-4 animate-spin bg-black'/>}*/ <p className='text-black text-center'>Loading...</p>
      : apiKeys.length === 0 ? (
          ...
          <li key={apiKey.key} className="p-4 border border-gray-200 rounded-md shadow-sm bg-gray-50">
            ...
            <ul className="list-disc pl-4 space-y-2">
              {apiKey.contexts.map((context) => (
                <li key={context.id} className="text-gray-600 flex gap-2">
                  <strong className='text-gray-700'>Name:</strong> <p className='text-black'>{context.name}</p> <br />
                </li>
              ))}
            </ul>
          </li>
